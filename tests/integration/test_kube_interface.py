...
from spark_client.domain import Defaults, PropertyFile
from spark_client.domain import KubernetesResourceType
...
class TestKubeInterface(TestCase):
    ...
    defaults = Defaults(
        dict(os.environ) | {"SNAP": "/snap/spark-client/current/"}
    )
    ...
    def test_create_exists_delete_secret(self):
        ...
        assert not k1.exists(KubernetesResourceType.SECRET_GENERIC, secret_name, namespace)
